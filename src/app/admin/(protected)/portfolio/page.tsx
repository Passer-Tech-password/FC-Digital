...
import { Plus, Edit, Trash2, Eye, Search, X, Loader2, FolderKanban } from "lucide-react";
...
const PortfolioAdminPage: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    if (!db || !confirm("Are you sure you want to delete this portfolio item?")) return;
    try {
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredItems.map((item) => (
                <div key={item.id} className="p-4 bg-card-800/50 rounded-xl border border-border-100">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button variant="ghost" size="icon" className="w-7 h-7 text-primary-400" onClick={() => handleEdit(item)}>
                        <Edit size={14} />
                      </Button>
                      <Button variant="ghost" size="icon" className="w-7 h-7 text-red-400" onClick={() => handleDelete(item.id)}>
                        <Trash2 size={14} />
                      </Button>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-card-800 border border-border-100 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-border-100 flex items-center justify-between">
              <h2 className="text-xl font-bold">{editingItem ? "Edit Project" : "Add New Project"}</h2>
